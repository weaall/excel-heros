...
export const BALANCE = Object.freeze({
  ...
  // `guardSec` — 쓰러진 동료가 없을 때 거는 '재고용 보장'의 유효 시간(초). 그동안 처음 쓰러지는 한 명이 즉시 복귀한다.
  REVIVE: { extraChance: 0.30, extraPerStar: 0.10, extraMax: 2, guardSec: 25 },
  /**
  ...
  ROLE_PASSIVE: {
    ...
    // **회복을 '있으면 차오름' 에서 '일해서 채움' 으로 옮긴다**(6-130).
    //
    // 이 게임에서 회복은 세 번 과잉으로 드러났다(6-115 능동 치유 폭주 · 6-116 보호막 · 6-129 쓰러짐 0).
    // 그런데 공급을 줄이려고 **적을 세게 만드는 건 6-95·6-112에서 전부 실패했다** — 처치 속도만 깎여
    // 진행이 느려지고 위험은 안 생겼다. 회복은 처치 속도에 관여하지 않으므로 여기가 맞는 손잡이다.
    //
    // 4시간 × 4판으로 훑은 결과, **관문 도달은 어떤 설정에서도 움직이지 않았다**(60단계 68분11초 고정,
    // 90단계 99~100분). 움직인 건 쓰러짐과 시체 시간뿐이다:
    //
    //   지금(오라 0.6+0.15★ · 치유 6%)        쓰러짐  76 · 시체 10.5% · 최고 154
    //   **오라 절반 · 치유 1.6배**             쓰러짐 100 · 시체 15.2% · 최고 155   ← 여기
    //   오라 1/3 · 치유 2배 · 기본 절반         쓰러짐 147 · 시체 14.4% · 최고 144 (지분은 191로 최고)
    //   오라 없음 · 치유 2.5배 · 기본 절반       쓰러짐 138 · 시체 15.8% · 최고 145 (지분 196)
    //
    // 더 깎으면 영구 지분은 더 오르지만 도달 단계가 154 → 144로 내려간다. 한 번에 거기까지 가지 않고
    // **진행에 아무 대가가 없는 지점**에서 멈춘다. 더 깎는 선택지는 위 표에 재어 둔 채로 남겨 둔다.
    healer: { regen: 0.003, perStar: 0.00075, castPct: 0.10 },  // 초당 최대 HP 회복(★5 0.3%→0.6%) · 능동 치유 10%
  ...
  SKILL_STAR: {
    ...
    // 필살기 기절의 **기본 길이**(초)와 ★당 증가분. 2초였을 때 ★3 필살기는 14초마다 전체를 2.6초
    // 얼려서 — 그것만으로 받는 피해가 16~18% 줄었다. 필살기가 벤치 상위인 이유의 절반이 여기였다.
    ultStun: 1.6,
    ...
    // 회복: 넘친 만큼 ★당 최대 HP 1.5%까지 보호막으로.
    //
    // 5% → 3% 로 줄였더니 **아무것도 안 바뀌었다**(+54 → +55). 이 보호막의 힘은 **크기가 아니라
    // 지속 시간**이기 때문이다: 6.8초 유지 / 12초 쿨다운 = 55% 의 시간 동안 켜져 있고, 그동안 들어오는
    // 피해가 보호막보다 작으면 크기를 아무리 줄여도 전부 막는다. 상한을 실제 피해량 아래로 내려야
    // 비로소 값이 움직인다 — 분당 파티 체력 0.5배가 들어올 때 12초면 한 명 최대 HP의 10%쯤이다.
    // 웰니스 데이는 **잃은 체력** 기준이라 넘치지 않는다(6-134) — 넘침을 보호막으로 바꾸던
    // `healShield` 는 더 이상 쓰이지 않는다. ★ 보너스는 **가장 많이 다친 한 명을 ★당 이만큼 더** 채운다.
    healFocus: 0.25,
    // 웰니스 데이를 **언제 쓸 것인가**.
    //
    // 나는 "★ 보너스(넘친 회복 → 보호막)는 만피일 때만 의미가 있는데 발동 조건이 만피를 막으니
    // 영영 못 쓴다"고 읽고 `healIdleShield` 를 켰다. **틀린 읽기였다.** 넘침은 만피에서만 생기는 게
    // 아니다 — 한 명이 다쳐서 시전하면 **나머지 넷의 회복이 그대로 넘쳐서** 보호막이 된다. 보너스는
    // 원래부터 잘 발동하고 있었다.
    //
    // 그리고 켜 놓은 대가가 컸다: 60단계 도달 64분 → 105분. 절반은 `castLock`(채우기 시전이 파티의
    // 진짜 스킬을 1.1초씩 밀어냈다 — 그건 따로 고쳤다), 나머지 절반은 **쿨다운을 미리 써 버려서
    // 정작 누가 다쳤을 때 힐이 준비돼 있지 않은 것**이다. 그래서 0으로 되돌린다.
    // 0.75 → 0.85. 힐이 **더 자주 터지되 만피에서는 안 터지게** 하는 지점이다. 3시간 × 3판:
    // 0.75 최고 117/쓰러짐 69 · **0.85 최고 120/쓰러짐 46** · 0.95 최고 122/쓰러짐 67.
    // 0.95는 사실상 '만피가 아니면 언제나'라서 채우기 시전의 대가(잠금·쿨다운 선소모)가 돌아온다.
    healWorth: 0.85,
    healIdleShield: 0,
  },
