...
const NEG = 'lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, cropped head, head out of frame, close-up, hair over eyes, covered face, hand over face, backlighting, silhouette, dark face, worst quality, low quality, jpeg artifacts, signature, watermark, username, blurry face, 3d, realistic, photo, multiple views, busy background, cluttered background, high contrast background, nsfw';
...
export function prompt(def, profileId) {
  ...
  return `${describe(def, profileId)}, looking at viewer, face fully visible, eyes visible, head in frame, cowboy shot, ${bg} (soft, out of focus), ${STYLE_TAGS}, masterpiece, best quality, very aesthetic, absurdres`;
}
