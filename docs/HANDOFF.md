...
## 5. 남은 일 (우선순위 순)
1. ~~수치 실플레이 검증~~ **완료(6-98)**: 여덟 시스템 전부 브라우저에서 실측했고 문서대로 작동한다 — 수식 대응 60.0% · 탱커 엄호 0.578(기대 0.580) · 힐러 오라 1.2%/s · 원거리 관통 0.387(끄면 0.000) · 근접 기세 ×1.28/2.5초 · ★스킬 ×1.495 · 스카우트 3회/일 · 인사 복구 ★1 1.87명→★5 2.77명. 계측할 때는 **아무 일도 안 했을 때 0이 나오는지 먼저 확인한다** — 없는 함수를 옵셔널 체이닝으로 불러 '스킬을 한 번도 안 쓰고' 그럴듯한 분포를 뽑은 적이 있다.
...
4. **애드센스 승인 후**: `EXCEL_HEROES_ADS.publisherId` 설정, 노출 빈도 조정, ads.txt 확인.
5. **HF 토큰 재발급**(사용자 작업) 후 `.hf_tokens` 갱신.
