...
## 5. 남은 일 (우선순위 순)
0. **[1순위 열린 질문] 회복이 과잉이다 — 세 번 같은 결론이 나왔다(6-129).** 기본 재생 0.4%/s + 힐러 오라 0.9%/s + 힐러 능동 치유 + 힐 스킬이 겹쳐서, 60단계 고정 전투 30분 × 4판에 **쓰러진 사람이 0명**이다. 그래서 탱커 치명타 엄호는 발동조차 안 하고 힐러 오라는 판정 불가다. 공급을 줄이면 생존 시스템이 전부 살아나지만, 난이도를 올리려던 과거 시도(6-95·6-112)는 전부 '진행만 느려지고 드라마는 안 생겼다'로 끝났다. **수치 하나가 아니라 설계 질문이므로 따로 다룰 것.**
