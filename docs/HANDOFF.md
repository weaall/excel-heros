...
## 5. 남은 일 (우선순위 순)
1. **주인공 직급 스킨 22장 재생성**: 만들긴 했는데 **절반가량이 멀리 선 작은 인물**이다. 원인은 `SKIN_BG` 가 넓은 장소('거리', '연회장')를 가리켜 모델이 뒤로 물러선 것 — 이미 `cafe window close behind him` / `warm party lights close behind him` 으로 좁혀 뒀으니 쿼터가 차면 `--skin intern staff sales_senior … admin` 으로 다시 뽑으면 된다.
2. **애드센스 승인 후**: `EXCEL_HEROES_ADS.publisherId` 설정, 노출 빈도 조정, ads.txt 확인.
3. **HF 토큰 재발급**(사용자 작업) 후 `.hf_tokens` 갱신.
4. **재창업 수치 실플레이 검증**: 지분 150주 조건 · 창업 경험 1점당 지분 효율 +25%/조각 +20%/뽑기 -3%. 한 사이클 뒤에 이득이 되도록 계산은 맞췄지만(6-79) 체감은 미검증.
5. **스카우트 수치 재검토**: 하루 3회 · 상한 비용의 35%(6-76).
6. **보스 스킬 체감 검증**: volley 0.6×3, slow 0.7/4s, throw 1.4×.
