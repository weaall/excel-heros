...
## 5. 남은 일 (우선순위 순)
0. ~~회복 과잉~~ **한 걸음 갔다(6-130)**: 오라를 절반으로, 힐러의 능동 치유를 6% → 10%로 옮겼다 — **관문 도달에 대가가 0인 지점**이다(60단계 68분11초 그대로, 최고 155 vs 154). 쓰러짐 76 → 100. 더 깎는 선택지도 재어 뒀다: 오라 1/3 + 치유 2배면 영구 지분이 172 → 191로 오르는 대신 도달 단계가 154 → 144로 내려간다. **다음에 이어서 볼 때 그 거래를 받을지 정하면 된다.**
   - 이때 쓴 손잡이가 중요하다: **적을 세게 만들면 처치 속도가 깎여 진행만 느려진다**(6-95·6-112에서 두 번 실패). 회복은 처치 속도에 관여하지 않아서 **진행을 안 건드리고 위험만 올릴 수 있는 유일한 자리**다.
1. **스킬 벤치를 다시 돌릴 때** — `node scripts/skillBench.mjs` (기본 60단계). 눈금(속도·체력·회복)을 건드렸으면 **반드시 다시 재고** 값을 고칠 것. 지금 표에서 바닥인 둘은 의도된 것이다: `heal`·`revive` 는 **반응형**이라 아무도 안 쓰러지는 싸움에서는 값을 못 한다.
2. **아직 상자인 보스는 없다** — 아홉 전부 다른 실루엣(6-122·6-125·6-127). 손 안 댄 건 0x72 팩 보스 3종(악마·좀비·오우거)뿐이고, 사무실 소품이 정체를 만들어 주고 있어 급하지 않다.
3. ~~스테이지 수식어 감사~~ **완료(6-129)**: 껐더니 60단계 도달이 **+6.0%** 늦어진다 — 살아 있다.
4. ~~수치 실플레이 검증~~ **완료(6-98)**: 여덟 시스템 전부 브라우저에서 실측했고 문서대로 작동한다 — 수식 대응 60.0% · 탱커 엄호 0.578(기대 0.580) · 힐러 오라 1.2%/s · 원거리 관통 0.387(끄면 0.000) · 근접 기세 ×1.28/2.5초 · ★스킬 ×1.495 · 스카우트 3회/일 · 인사 복구 ★1 1.87명→★5 2.77명. 계측할 때는 **아무 일도 안 했을 때 0이 나오는지 먼저 확인한다** — 없는 함수를 옵셔널 체이닝으로 불러 '스킬을 한 번도 안 쓰고' 그럴듯한 분포를 뽑은 적이 있다.
...
5. **0x72 보스 3종(악마·좀비·오우거)**: 재확인해 보니 **픽셀 밀도는 이미 9종 모두 3px 로 같다**(팩 32px×3, 손그림 43격자×3). 남은 차이는 몸통이 던전 생물이라는 것뿐이고, 사무실 소품이 정체를 만들어 주고 있어 급하지 않다. 사람 보스(야근 좀비 부장)는 오히려 있는 편이 낫다.
6. **지적된 카드 14장 재생성 미완**: `staff_park parttime courier intern_seo mail_cho macro design_lead chro cdo ceo founder ai_lead chief_of_staff union_chief`. 프롬프트 수정(헤일로 사다리 · 자세 · 설명 정리)은 커밋됐고 **그림은 아직 옛것**이다. Space 가 `No GPU was available after 60s` 로 막혀 한 장도 못 뽑았다. 다시 시도:
   ```
...
7. **애드센스 승인 후**: `EXCEL_HEROES_ADS.publisherId` 설정, 노출 빈도 조정, ads.txt 확인.
8. **HF 토큰 재발급**(사용자 작업) 후 `.hf_tokens` 갱신.
9. 열린 질문(BALANCE 8장): 부장 트랙 "전직" 기능 필요 여부, 순위표 노출 정책 등.

### 자尺부터 의심한다 (이 프로젝트에서 다섯 번 틀렸다)

| # | 무엇이 틀렸나 | 어떻게 들켰나 |
| --- | --- | --- |
| 6-97 | 난수 미고정 · `maxCleared`(이전이 되돌린다) · 순위표 점수(눈금 50%) | **모든** 변종이 기준선보다 좋게 나왔다 |
| 6-98 | 없는 함수를 옵셔널 체이닝으로 불러 아무것도 안 하고 그럴듯한 분포를 뽑았다 | — (하마터면 보고할 뻔했다) |
| 6-114 | `prestige()` 가 `this.entities` 를 갈아치워 인스턴스 패치가 사라졌다 | 같은 일을 하는 두 측정의 **부호가 반대**였다 |
| 6-123 | 기준선이 스킬을 1814회 발동했다(0이어야 한다) — `refreshHeroStats` 가 되돌린다 | **아무 일도 안 했을 때 0인지** 확인해서 |
| 6-132 | 복원이 스냅샷을 BALANCE 안으로 **별칭**시켜, 둘째 줄부터 앞서 끈 시스템이 계속 꺼져 있었다 | 서로 다른 두 시스템이 **소수점까지 같은 숫자**를 냈다 |

세 가지 습관이 이걸 다 잡았다:
- **아무 일도 안 했을 때 지표가 0을 찍는지 먼저 본다.**
- **불가능한 결과(버프를 껐는데 좋아짐 / 두 변종이 완전히 같음)가 보이면 게임이 아니라 자尺를 본다.**
- **한 관문에서 평평하다고 전 구간이 평평한 게 아니다**(6-132에서 이걸로 한 번 단정했다가 취소했다).

### 재는 도구 세 개 (어느 것을 언제 쓰는가)

| 도구 | 재는 것 | 못 재는 것 |
| --- | --- | --- |
| `scripts/abSystems.mjs` | 시스템을 껐을 때 **관문 도달 시간**이 얼마나 밀리는지 | 초반에 갈리지 않는 시스템(탱커 치명타 엄호·힐러 오라). 뽑기·이전의 요동이 커서 3판으로는 ±10% 가 흔들린다 |
| `scripts/skillBench.mjs` | 스킬 한 종류의 값. **진행을 전부 끄고** 고정 단계에서 분당 처치·분당 받은 피해 | 반응형 스킬(회복·복직)은 아무도 안 쓰러지는 싸움에서 값이 0으로 나온다 — 그게 맞는 값이다 |
| `scripts/sysBench.mjs` | 시스템을 고정 단계에서 (탱커 엄호·보호막·오라 …) | 아무도 안 쓰러지는 싸움에서는 방어 시스템이 발동조차 안 한다 |

**셋 다 '받은 피해'는 `entities.heroDamageTaken`(게임이 센 원래 피해)을 읽는다.** 밖에서 체력 변화를
적분하면 같은 틱의 회복이 상쇄해서 **회복 계열을 아예 못 잰다** — 벤치 둘이 그 자尺로 "힐은 기여 0"을
찍고 있었다(6-134). 그리고 고친 뒤에도 **회복은 '받은 피해' 칸에 안 잡힌다**(막는 게 아니라 되돌리니까).
회복이 잡히는 칸은 **쓰러짐**이고, 그래서 `LEVEL_DROP=10` 으로 **질 수도 있는 싸움**을 만들어 재야 한다.
