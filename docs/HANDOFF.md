# 인수인계 (2026-09-17, 64차까지) — 다음 세션이 이어서 볼 문서

작업 규칙·보안·명령·함정은 `CLAUDE.md`(자동 로드). 이 문서는 **현재 상태와 남은 일**.

## 1. 한 줄 요약
Excel 문서로 위장한 방치형 픽셀 RPG + 가챠. 50명 영웅(D~S) + 주인공 11직급, 일러 161장(기본 61 + 스킨 100) + 프롤로그 패널 7장, Google 로그인 필수, 첫 로그인 오프닝 스토리, Cloudflare Worker + D1 백엔드, GitHub Pages 자동 배포. 테스트 103개 통과. 64차 패스까지 완료.
...
- `src/data/prologue.js`(오프닝 7장면), `src/data/heroes.js`(HEROES 50, MAIN_JOBS 11, MAIN_TRACKS, SKILLS, TRAITS, GRADES), `dollSprites.js`(도트 스펙 61 + `applySkin`: 스킨이 복장·액세서리까지 교체), `skins.js`, `pickup.js`, `divisions.js`(부문 시너지·특성), `profiles.js`+`profilesExtra.js`, `story.js`, `quips.js`, `monsters.js`(BOSSES specials), `packSprites.js`(0x72 몬스터), `cardArt.js`(매니페스트 로드, 썸네일/원본), `artPalettes.js`(자동 생성).
- `src/config/balance.js` — 모든 수치(AD_OFFERS, MAIN_PROMOTE_*, GEM_DROP, DISPATCH, OVERTIME, AFFECTION, SKILL_LEVEL …).
- `scripts/` — `genCardsHF.mjs`(일러, --skin, --scene, --manifest, 토큰 풀), `thumbs.py`(WebP), `extractPalettes.mjs`, `artSheet.mjs`, `dollSheet.mjs`, `serve.js`, `png.mjs`.
- `tests/` — 103개(`node --test`). 새 기능마다 테스트 추가가 관례.
...
## 4-1. 62~64차 (이번 세션)
- A/B 등급 일러 상향: 등급 태그 자체를 손봐(A = gold accents/sparkles, B = soft light particles, 전 등급 beautiful detailed face) CFO·CTO·CHRO·디자인팀장·기획팀장 재생성 + 도트 색 맞춤. 마스크가 얼굴을 덮는 문제는 네거티브로 차단하고 CDO 재생성.
- 스킬 컷인 재연출: 왼쪽에서 슬램 인하는 평행사변형 대형 초상 + 기울어진 명찰(사용자: "샥 뜨고 마는 건 맛이 없다").
- 프롤로그: `src/data/prologue.js` 7장면 + `--scene` 생성 모드(assets/story/<id>.png → webp) + 전체 화면 플레이어(`UIManager.showPrologue`), 첫 로그인 시 자동 1회(`settings.prologueSeen`), 사내_메신저 시트에서 다시 보기.
- 스킨이 도트 복장까지 교체. 도트 페인터 버그 2건(금장식이 외곽선에 덮임, 카디건이 정장과 동일) 수정.

## 5. 남은 일 (우선순위 순)
1. **애드센스 승인 후**: `EXCEL_HEROES_ADS.publisherId` 설정, 실제 노출 빈도(30s 힌트) 조정, ads.txt 확인.
2. **주인공 스킨 일러**: `--skin`은 기본적으로 HEROES만. 메인 직급 11개의 스킨 일러(22장)는 미생성(필요하면 `--skin intern staff …`).
3. **보스 스킬 밸런스 확인**: volley 0.6×3, slow 0.7/4s, throw 1.4× — 실제 플레이 체감 미검증(시뮬 테스트만).
4. **HF 토큰 재발급** 후 `.hf_tokens` 갱신(사용자 작업). 토큰이 4계정이면 하루 약 40~50장.
